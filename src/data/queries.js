...
export async function getFarmer() {
  ...
  const { data, error } = await supabase
    ...
    .single();
  return error || !data ? FARMER : data;
}

export const getListings = () => safeFetch("listings", LISTINGS);
export const getOrders = () => safeFetch("orders", ORDERS, (q) => q.order("created_at", { ascending: false }));
export const getNotifications = () => safeFetch("notifications", NOTIFICATIONS, (q) => q.order("created_at", { ascending: false }).limit(10));
export const getInsights = () => safeFetch("insights", INSIGHTS);

export async function getDelivery() {
  const rows = await safeFetch("deliveries", [DELIVERY], (q) => q.order("updated_at", { ascending: false }).limit(1));
  return Array.isArray(rows) ? rows[0] : rows;
}

// Computed stats — replaces the 4 hardcoded StatCards
